...
SCOPE_PASS = '{"result": "PASS", "reason": "This is an ML paper."}'
SCOPE_FAIL = '{"result": "FAIL", "reason": "Not an ML paper."}'


@patch("backend.nodes.node1_ingestor.pdf_part_from_gcs")
@patch("backend.nodes.node1_ingestor.get_pro_model")
@patch("backend.nodes.node1_ingestor.run_node0")
def test_node1_extracts_blueprint(mock_node0, mock_get_model, mock_pdf_part):
    ...
    result = run_node1("gs://test-bucket/papers/abc/paper.pdf")
...
@patch("backend.nodes.node1_ingestor.get_pro_model")
@patch("backend.nodes.node1_ingestor.run_node0")
def test_node1_strips_markdown_fences(mock_node0, mock_get_model, mock_pdf_part):
    mock_node0.return_value = MagicMock(result="PASS", reason="ML paper")
    mock_response = MagicMock()
