...
async def test_node0_pass(mock_get_model, mock_pdf_part):
    ...
    result = await run_node0("gs://test-bucket/papers/abc/paper.pdf")
...
@patch("backend.nodes.node0_validator.pdf_part_from_gcs")
@patch("backend.nodes.node0_validator.get_flash_model")
async def test_node0_fail(mock_get_model, mock_pdf_part):
    mock_response = MagicMock()
...
@patch("backend.nodes.node0_validator.pdf_part_from_gcs")
@patch("backend.nodes.node0_validator.get_flash_model")
async def test_node0_strips_markdown_fences(mock_get_model, mock_pdf_part):
    mock_response = MagicMock()
